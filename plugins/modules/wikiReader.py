...
def read_wiki_templates(wikitext, template_name, keep_markup = False):
    ...
    for t in template_objects:
        ret.append([t.string, t.name.strip()] + [str(a)[1:].strip() for a in t.arguments])
    return ret
...
def read_all_wiki_lists(wikitext, keep_markup = False, include_sublists = False):
    ...
    try:
        list_index = 0
        while True:
            res = res + read_wiki_list(wikitext, list_index=list_index, keep_markup=True, include_sublists=include_sublists)
            list_index += 1
    except:
        return res
