...
class MapCSS_ru_housenumber(Plugin):
    ...
    def init(self, logger):
        ...
        tags = capture_tags = {}
        self.errors[1] = {'item': 9017, 'level': 2, 'tag': [], 'desc': mapcss.tr(capture_tags, u'Номера домов не соответствующие принятому соглашению')}
    ...
    def node(self, data, tags, *args):
        ...
        if (u'addr:housenumber' in keys) and \
            ((mapcss._tag_capture(capture_tags, 0, tags, u'addr:housenumber') and not mapcss.regexp_test(self.re_62d22c1b, mapcss._tag_capture(capture_tags, 1, tags, u'addr:housenumber')))):
            # throwWarning:tr("Номера домов не соответствующие принятому соглашению")
            err.append({'class': 1, 'subclass': 774061168, 'text': mapcss.tr(capture_tags, u'Номера домов не соответствующие принятому соглашению')})
