...
class MapCSS_josm_ru_housenumber(Plugin):
    ...
    def init(self, logger):
        ...
        tags = capture_tags = {}
        self.errors[9017001] = {'item': 9017, 'level': 3, 'tag': [], 'desc': mapcss.tr(u'Номера домов не соответствующие принятому соглашению', capture_tags)}
    ...
    def node(self, data, tags, *args):
        ...
        if u'addr:housenumber' in keys:
            match = False
            try: match = match or ((mapcss._tag_capture(capture_tags, 0, tags, u'addr:housenumber') and not mapcss.regexp_test_(self.re_62d22c1b, mapcss._tag_capture(capture_tags, 1, tags, u'addr:housenumber'))))
