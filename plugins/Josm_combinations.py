...
class Josm_combinations(PluginMapCSS):
    ...
    def node(self, data, tags):
        ...
        keys = tags.keys()
        err = []
        set_AllSameCycleway = set_AllSameMaxspeed = set_AllSameSidewalk = set_MotorwayTrunk = set_PlacementTransitionWarning = set_fully_paved_surface = set_only_one_tag = set_unpaved_surface = False
        ...
        if ('artwork_type' in keys) or ('board_type' in keys) or ('bunker_type' in keys) or ('castle_type' in keys) or ('fire_hydrant:type' in keys) or ('generator:method' in keys) or ('generator:source' in keys) or ('generator:type' in keys) or ('information' in keys) or ('lamp_type' in keys) or ('manhole' in keys) or ('map_type' in keys) or ('parking' in keys) or ('railway:switch' in keys) or ('recycling_type' in keys) or ('shelter_type' in keys) or ('site_type' in keys) or ('zoo' in keys):
            ...
            if match:
                ...
                err.append({'class': 9001001, 'subclass': 1216906377, 'text': mapcss.tr('{0} without {1}', mapcss._tag_uncapture(capture_tags, '{0.key}'), mapcss._tag_uncapture(capture_tags, '{1.tag}'))})
        ...
        if ('actuator' in keys and 'handle' in keys) or ('addr:housenumber' in keys and 'nohousenumber' in keys) or ('amenity' in keys and 'shop' in keys) or ('frequency' in keys and 'power' in keys) or ('handle' in keys and 'mechanical_driver' in keys) or ('internet_access' in keys and 'internet_access:fee' in keys) or ('name' in keys and 'noname' in keys) or ('noref' in keys and 'ref' in keys) or ('power' in keys) or ('power' in keys and 'transformer' in keys and 'voltage' in keys) or ('power' in keys and 'voltage' in keys):
            ...
            if match:
                ...
                err.append({'class': 9001002, 'subclass': 1617348591, 'text': mapcss.tr('{0} together with {1}', mapcss._tag_uncapture(capture_tags, '{0.tag}'), mapcss._tag_uncapture(capture_tags, '{1.key}'))})
        ...
        # *[tunnel][!highway][!area:highway][!railway][!waterway][!piste:type][type!=tunnel][public_transport!=platform][route!=ferry][man_made!=pipeline][man_made!=goods_conveyor][man_made!=wildlife_crossing][man_made!=tunnel][power!=cable]
        ...
        if ('bridge' in keys) or ('lanes' in keys) or ('maxspeed' in keys) or ('psv' in keys) or ('tunnel' in keys) or ('width' in keys):
            ...
            if match:
                ...
                err.append({'class': 9001002, 'subclass': 1679285684, 'text': mapcss.tr('{0} on suspicious object', mapcss._tag_uncapture(capture_tags, '{0.key}'))})
        ...
        if ('sport' in keys):
            ...
            if match:
                ...
                err.append({'class': 9001001, 'subclass': 2099444702, 'text': mapcss.tr('sport without physical feature')})
    ...
    def way(self, data, tags, nds):
        ...
        if ('border_type' in keys) or ('canal' in keys) or ('detail' in keys) or ('eddy_current_brake' in keys) or ('electrified' in keys) or ('etcs' in keys) or ('gauge' in keys) or ('grade_of_track' in keys) or ('have_riverbank' in keys) or ('junction' in keys) or ('kursbuchstrecke' in keys) or ('length_unit' in keys) or ('living_street' in keys) or ('lzb' in keys) or ('maintenance' in keys) or ('median' in keys) or ('motorroad' in keys) or ('old_railway_operator' in keys) or ('operating_procedure' in keys) or ('piste:difficulty' in keys) or ('place' in keys) or ('pzb' in keys) or ('sac_scale' in keys) or ('sidewalk' in keys) or ('source:addr:housenumber' in keys) or ('source:addr:postcode' in keys) or ('source:bicycle' in keys) or ('source:bridge' in keys) or ('source:building' in keys) or ('source:date' in keys) or ('source:designation' in keys) or ('source:ele' in keys) or ('source:height' in keys) or ('source:hgv' in keys) or ('source:highway' in keys) or ('source:housenumber' in keys) or ('source:lanes' in keys) or ('source:lit' in keys) or ('source:maxaxleload' in keys) or ('source:maxspeed:backward' in keys) or ('source:maxspeed:forward' in keys) or ('source:name' in keys) or ('source:old_name' in keys) or ('source:population' in keys) or ('source:postal_code' in keys) or ('source:postcode' in keys) or ('source:ref' in keys) or ('source:ref:INSEE' in keys) or ('source:surface' in keys) or ('step_count' in keys) or ('structure_gauge' in keys) or ('tilting_technology' in keys) or ('track_class' in keys) or ('tracks' in keys) or ('tracktype' in keys) or ('traffic_mode' in keys) or ('trail_visibility' in keys) or ('transformer' in keys) or ('trolley_wire' in keys) or ('workrules' in keys) or ('zip_left' in keys) or ('zip_right' in keys):
            ...
            if match:
                # group:tr("missing tag")
                # throwWarning:tr("{0} without {1}","{0.key}","{1.key}")
                err.append({'class': 9001001, 'subclass': 1452726368, 'text': mapcss.tr('{0} without {1}', mapcss._tag_uncapture(capture_tags, '{0.key}'), mapcss._tag_uncapture(capture_tags, '{1.key}'))})
        ...
        # *[zoo][tourism!=zoo]
        # *[bunker_type][military!=bunker]
        if ('artwork_type' in keys) or ('bunker_type' in keys) or ('castle_type' in keys) or ('cutline' in keys) or ('fence_type' in keys) or ('generator:method' in keys) or ('generator:source' in keys) or ('generator:type' in keys) or ('information' in keys) or ('parking' in keys) or ('recycling_type' in keys) or ('shelter_type' in keys) or ('site_type' in keys) or ('zoo' in keys):
            match = False
            if not match:
                capture_tags = {}
                try: match = ((mapcss._tag_capture(capture_tags, 0, tags, 'generator:source')) and (mapcss._tag_capture(capture_tags, 1, tags, 'power') != mapcss._value_const_capture(capture_tags, 1, 'generator', 'generator')))
            ...
            if match:
                # group:tr("missing tag")
                # throwWarning:tr("{0} without {1}","{0.key}","{1.tag}")
                err.append({'class': 9001001, 'subclass': 1847752037, 'text': mapcss.tr('{0} without {1}', mapcss._tag_uncapture(capture_tags, '{0.key}'), mapcss._tag_uncapture(capture_tags, '{1.tag}'))})
        ...
        # *[amenity=place_of_worship][!religion]
        # *[man_made=tower][!tower:type]
        if ('actuator' in keys) or ('aeroway' in keys) or ('amenity' in keys) or ('boundary' in keys) or ('highway' in keys) or ('leisure' in keys) or ('man_made' in keys) or ('mechanical_driver' in keys) or ('office' in keys) or ('power' in keys) or ('railway' in keys) or ('tourism' in keys):
            match = False
            if not match:
                capture_tags = {}
            ...
            if match:
                # group:tr("missing tag")
                # throwWarning:tr("{0} without {1}","{0.tag}","{1.key}")
                err.append({'class': 9001001, 'subclass': 1304064328, 'text': mapcss.tr('{0} without {1}', mapcss._tag_uncapture(capture_tags, '{0.tag}'), mapcss._tag_uncapture(capture_tags, '{1.key}'))})
        ...
        if ('actuator' in keys and 'handle' in keys) or ('addr:housenumber' in keys and 'nohousenumber' in keys) or ('amenity' in keys and 'shop' in keys) or ('frequency' in keys and 'power' in keys) or ('handle' in keys and 'mechanical_driver' in keys) or ('internet_access' in keys and 'internet_access:fee' in keys) or ('name' in keys and 'noname' in keys) or ('noref' in keys and 'ref' in keys) or ('oneway' in keys) or ('power' in keys) or ('power' in keys and 'voltage' in keys):
            ...
            if match:
                ...
                err.append({'class': 9001002, 'subclass': 339623562, 'text': mapcss.tr('{0} together with {1}', mapcss._tag_uncapture(capture_tags, '{0.tag}'), mapcss._tag_uncapture(capture_tags, '{1.key}'))})
        ...
        if ('amenity' in keys and 'police' in keys) or ('barrier' in keys and 'kerb' in keys) or ('bicycle' in keys and 'segregated' in keys) or ('bridge' in keys and 'man_made' in keys) or ('construction' in keys and 'highway' in keys) or ('construction' in keys and 'railway' in keys) or ('foot' in keys and 'segregated' in keys) or ('highway' in keys and 'junction' in keys) or ('man_made' in keys and 'tunnel' in keys) or ('surface' in keys and 'tracktype' in keys):
            ...
            if match:
                # group:tr("suspicious tag combination")
                # throwWarning:tr("{0} together with {1}","{0.tag}","{1.tag}")
                err.append({'class': 9001002, 'subclass': 1163066778, 'text': mapcss.tr('{0} together with {1}', mapcss._tag_uncapture(capture_tags, '{0.tag}'), mapcss._tag_uncapture(capture_tags, '{1.tag}'))})
        ...
        if ('bridge' in keys) or ('incline' in keys) or ('lanes' in keys) or ('maxspeed' in keys) or ('oneway' in keys) or ('psv' in keys) or ('tunnel' in keys) or ('width' in keys):
            ...
            if match:
                # group:tr("suspicious tag combination")
                # throwWarning:tr("{0} on suspicious object","{0.key}")
                err.append({'class': 9001002, 'subclass': 508481893, 'text': mapcss.tr('{0} on suspicious object', mapcss._tag_uncapture(capture_tags, '{0.key}'))})
        ...
        if ('water' in keys):
            ...
            if match:
                # group:tr("missing tag")
                # throwWarning:tr("{0} without {1}","{0.key}","{1.tag}")
                err.append({'class': 9001001, 'subclass': 1200494527, 'text': mapcss.tr('{0} without {1}', mapcss._tag_uncapture(capture_tags, '{0.key}'), mapcss._tag_uncapture(capture_tags, '{1.tag}'))})
        ...
        if ('construction' in keys and 'highway' in keys) or ('construction' in keys and 'railway' in keys):
            match = False
            if not match:
                capture_tags = {}
            ...
            if match:
                # group:tr("suspicious tag combination")
                # throwWarning:tr("{0} together with {1}","{0.tag}","{1.tag}")
                err.append({'class': 9001002, 'subclass': 1030349168, 'text': mapcss.tr('{0} together with {1}', mapcss._tag_uncapture(capture_tags, '{0.tag}'), mapcss._tag_uncapture(capture_tags, '{1.tag}'))})
    ...
    def relation(self, data, tags, members):
        ...
        if ('border_type' in keys) or ('piste:difficulty' in keys) or ('place' in keys) or ('source:addr:housenumber' in keys) or ('source:addr:postcode' in keys) or ('source:bicycle' in keys) or ('source:bridge' in keys) or ('source:building' in keys) or ('source:date' in keys) or ('source:designation' in keys) or ('source:ele' in keys) or ('source:height' in keys) or ('source:hgv' in keys) or ('source:highway' in keys) or ('source:housenumber' in keys) or ('source:lanes' in keys) or ('source:lit' in keys) or ('source:maxaxleload' in keys) or ('source:maxspeed:backward' in keys) or ('source:maxspeed:forward' in keys) or ('source:name' in keys) or ('source:old_name' in keys) or ('source:population' in keys) or ('source:postal_code' in keys) or ('source:postcode' in keys) or ('source:ref' in keys) or ('source:ref:INSEE' in keys) or ('source:surface' in keys) or ('transformer' in keys):
            ...
            if match:
                # group:tr("missing tag")
                # throwWarning:tr("{0} without {1}","{0.key}","{1.key}")
                err.append({'class': 9001001, 'subclass': 1063128178, 'text': mapcss.tr('{0} without {1}', mapcss._tag_uncapture(capture_tags, '{0.key}'), mapcss._tag_uncapture(capture_tags, '{1.key}'))})
        ...
        # *[zoo][tourism!=zoo]
        # *[bunker_type][military!=bunker]
        if ('artwork_type' in keys) or ('bunker_type' in keys) or ('castle_type' in keys) or ('generator:method' in keys) or ('generator:source' in keys) or ('generator:type' in keys) or ('information' in keys) or ('parking' in keys) or ('recycling_type' in keys) or ('shelter_type' in keys) or ('site_type' in keys) or ('zoo' in keys):
            match = False
            if not match:
                capture_tags = {}
                try: match = ((mapcss._tag_capture(capture_tags, 0, tags, 'generator:source')) and (mapcss._tag_capture(capture_tags, 1, tags, 'power') != mapcss._value_const_capture(capture_tags, 1, 'generator', 'generator')))
            ...
            if match:
                # group:tr("missing tag")
                # throwWarning:tr("{0} without {1}","{0.key}","{1.tag}")
                err.append({'class': 9001001, 'subclass': 957877692, 'text': mapcss.tr('{0} without {1}', mapcss._tag_uncapture(capture_tags, '{0.key}'), mapcss._tag_uncapture(capture_tags, '{1.tag}'))})
        ...
        # *[amenity=place_of_worship][!religion]
        # *[man_made=tower][!tower:type]
        if ('actuator' in keys) or ('aeroway' in keys) or ('amenity' in keys) or ('boundary' in keys) or ('leisure' in keys) or ('man_made' in keys) or ('mechanical_driver' in keys) or ('office' in keys) or ('power' in keys) or ('route' in keys and 'type' in keys) or ('tourism' in keys):
            match = False
            if not match:
                capture_tags = {}
                try: match = ((mapcss._tag_capture(capture_tags, 0, tags, 'actuator') == mapcss._value_capture(capture_tags, 0, 'manual')) and (not mapcss._tag_capture(capture_tags, 1, tags, 'handle')))
            ...
            if match:
                # group:tr("missing tag")
                # throwWarning:tr("{0} without {1}","{0.tag}","{1.key}")
                err.append({'class': 9001001, 'subclass': 2053738729, 'text': mapcss.tr('{0} without {1}', mapcss._tag_uncapture(capture_tags, '{0.tag}'), mapcss._tag_uncapture(capture_tags, '{1.key}'))})
        ...
        if ('actuator' in keys and 'handle' in keys) or ('addr:housenumber' in keys and 'nohousenumber' in keys) or ('amenity' in keys and 'shop' in keys) or ('frequency' in keys and 'power' in keys) or ('handle' in keys and 'mechanical_driver' in keys) or ('internet_access' in keys and 'internet_access:fee' in keys) or ('name' in keys and 'noname' in keys) or ('noref' in keys and 'ref' in keys) or ('power' in keys) or ('power' in keys and 'voltage' in keys):
            ...
            if match:
                # group:tr("suspicious tag combination")
                # throwWarning:tr("{0} together with {1}","{0.tag}","{1.key}")
                err.append({'class': 9001002, 'subclass': 1844457543, 'text': mapcss.tr('{0} together with {1}', mapcss._tag_uncapture(capture_tags, '{0.tag}'), mapcss._tag_uncapture(capture_tags, '{1.key}'))})
        ...
        if ('bridge' in keys) or ('maxspeed' in keys) or ('psv' in keys) or ('tunnel' in keys) or ('width' in keys):
            ...
            if match:
                # group:tr("suspicious tag combination")
                # throwWarning:tr("{0} on suspicious object","{0.key}")
                err.append({'class': 9001002, 'subclass': 1888816461, 'text': mapcss.tr('{0} on suspicious object', mapcss._tag_uncapture(capture_tags, '{0.key}'))})
...
class Test(TestPluginCommon):
    def test(self):
        ...
        self.check_err(n.node(data, {'source:addr:postcode': 'postman'}), expected={'class': 9001001, 'subclass': 31231113}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'maxspeed:hgv:conditional': '80 @ (weight>7.5 AND 22:00-05:00)', 'traffic_sign': 'maxspeed'}), expected={'class': 9001001, 'subclass': 2095780217}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'addr:postcode': '12345', 'place': 'foo'}), expected={'class': 9001002, 'subclass': 1053814004}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'addr:housenumber': '5', 'addr:postcode': '12345', 'place': 'foo'}), expected={'class': 9001002, 'subclass': 1053814004}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'addr:housenumber': '5', 'place': 'foo'}), expected={'class': 9001002, 'subclass': 1053814004}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'highway': 'street_lamp', 'natural': 'birds_nest'}), expected={'class': 9001002, 'subclass': 1750941961}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'amenity': 'restaurant', 'sport': '10pin'}), expected={'class': 9001001, 'subclass': 2099444702}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'natural': 'beach', 'sport': 'beachvolleyball'}), expected={'class': 9001001, 'subclass': 2099444702}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'sport': 'skiing'}), expected={'class': 9001001, 'subclass': 2099444702}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'sport': 'swimming', 'tourism': 'hotel'}), expected={'class': 9001001, 'subclass': 2099444702}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'leisure': 'pitch', 'sport': 'tennis'}), expected={'class': 9001001, 'subclass': 2099444702}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'sport': 'tennis'}), expected={'class': 9001001, 'subclass': 2099444702}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'amenity': 'recycling', 'collection_times': 'Mo 08:00-11:00'}), expected={'class': 9001002, 'subclass': 1009884322}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'amenity': 'recycling', 'collection_times': 'Mo-Fr 06:00-20:00'}), expected={'class': 9001002, 'subclass': 1009884322}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'amenity': 'recycling', 'collection_times': 'Mo-Fr 15:00'}), expected={'class': 9001002, 'subclass': 1009884322}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'amenity': 'recycling', 'collection_times': 'Sa[2,4] 8:00-11:00'}), expected={'class': 9001002, 'subclass': 1009884322}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'addr:housenumber': '42', 'source:addr': 'postman'}), expected={'class': 9001001, 'subclass': 886065920}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'source:addr': 'postman'}), expected={'class': 9001001, 'subclass': 886065920}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'height': '4358\''}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'height': '245\'', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'height': '224.22 ft', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'height': '328.22 ft', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'height': '74 m', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'height': '75 m', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'height': '0.8', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'height': '231\'10.22"', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'height': '4358\'8"', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.node(data, {'height': '58', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.node(data, {'height': '75.72', 'man_made': 'communications_tower'}), expected={'class': 9001002, 'subclass': 467978856}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'plant:source': 'combustion', 'power': 'generator'}, [0]), expected={'class': 9001002, 'subclass': 339623562}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'generator:source': 'wind', 'power': 'plant'}, [0]), expected={'class': 9001002, 'subclass': 339623562}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'alt_name': 'Foo', 'name': 'Bar'}, [0]), expected={'class': 9001001, 'subclass': 1822630838}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'alt_name': 'Foo'}, [0]), expected={'class': 9001001, 'subclass': 1822630838}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'lanes': '2'}, [0]), expected={'class': 9001001, 'subclass': 1503842400}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'lanes': '3', 'lanes:backward': '2'}, [0]), expected={'class': 9001001, 'subclass': 1503842400}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'lanes': '3', 'oneway': '-1'}, [0]), expected={'class': 9001001, 'subclass': 1503842400}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'primary', 'lanes': '3'}, [0]), expected={'class': 9001001, 'subclass': 1503842400}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'lanes': '4'}, [0]), expected={'class': 9001001, 'subclass': 1503842400}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'primary', 'turn:lanes:forward': 'left|right'}, [0]), expected={'class': 9001001, 'subclass': 1407445006}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'lanes': '2', 'turn:lanes': 'left|right'}, [0]), expected={'class': 9001001, 'subclass': 1407445006}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'primary', 'turn:lanes': 'left|right'}, [0]), expected={'class': 9001001, 'subclass': 1407445006}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'pedestrian', 'width': '0.8'}, [0]), expected={'class': 9001002, 'subclass': 867332242}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'pedestrian', 'width': '1'}, [0]), expected={'class': 9001002, 'subclass': 867332242}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'pedestrian', 'width': '3'}, [0]), expected={'class': 9001002, 'subclass': 867332242}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'pedestrian', 'width': '5.5'}, [0]), expected={'class': 9001002, 'subclass': 867332242}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'building': 'house', 'construction:building': 'house'}, [0]), expected={'class': 9001001, 'subclass': 1239539337}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'construction': 'house', 'construction:building': 'house'}, [0]), expected={'class': 9001001, 'subclass': 1239539337}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'construction:building': 'house'}, [0]), expected={'class': 9001001, 'subclass': 1239539337}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'building': 'construction', 'construction:building': 'house'}, [0]), expected={'class': 9001001, 'subclass': 855187849}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'building': 'house', 'construction:building': 'house'}, [0]), expected={'class': 9001001, 'subclass': 855187849}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'construction': 'house', 'construction:building': 'house'}, [0]), expected={'class': 9001001, 'subclass': 855187849}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'building': 'construction', 'construction:building': 'house'}, [0]), expected={'class': 9001002, 'subclass': 164864338}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'building': 'office', 'construction:building': 'house'}, [0]), expected={'class': 9001002, 'subclass': 164864338}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'construction': 'house', 'construction:building': 'house'}, [0]), expected={'class': 9001002, 'subclass': 164864338}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'construction': 'house', 'construction:building': 'house'}, [0]), expected={'class': 9001002, 'subclass': 1971600459}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'construction': 'office', 'construction:building': 'house'}, [0]), expected={'class': 9001002, 'subclass': 1971600459}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'construction': 'footway', 'crossing': 'unmarked', 'highway': 'construction'}, [0]), expected={'class': 9001002, 'subclass': 1696418751}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'crossing': 'unmarked', 'highway': 'trunk'}, [0]), expected={'class': 9001002, 'subclass': 1696418751}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'fixme': 'name'}, [0]), expected={'class': 9001002, 'subclass': 2092275873}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'FIXME': 'name', 'name': 'Florist Gump'}, [0]), expected={'class': 9001002, 'subclass': 2092275873}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'fixme': 'the name might have changed', 'name': 'Florist Gump'}, [0]), expected={'class': 9001002, 'subclass': 2092275873}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'fixme': 'name', 'name': 'Florist Gump'}, [0]), expected={'class': 9001002, 'subclass': 2092275873}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'name': 'Florist Gump'}, [0]), expected={'class': 9001002, 'subclass': 2092275873}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'placement:backward': 'middle_of:1', 'placement:forward': 'transition'}, [0]), expected={'class': 9001005, 'subclass': 942326561}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'primary', 'placement:backward': 'transition', 'placement:forward': 'transition'}, [0]), expected={'class': 9001005, 'subclass': 942326561}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'placement': 'middle_of:1', 'placement:backward': 'transition', 'placement:forward': 'transition'}, [0]), expected={'class': 9001005, 'subclass': 942326561}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'primary', 'placement': 'transition', 'placement:both_ways': 'transition'}, [0]), expected={'class': 9001005, 'subclass': 942326561}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'placement:forward': 'right_of:1'}, [0]), expected={'class': 9001002, 'subclass': 2100246844}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'primary', 'placement:both_ways': 'transition', 'placement:forward': 'transition'}, [0]), expected={'class': 9001002, 'subclass': 2100246844}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'primary', 'placement': 'left_of:2', 'placement:forward': 'right_of:1'}, [0]), expected={'class': 9001002, 'subclass': 2100246844}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'abandoned:highway': 'unclassified', 'footway': 'sidewalk'}, [0]), expected={'class': 9001001, 'subclass': 1698700242}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'bicycle': 'no', 'cycleway:right': 'no'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'bicycle': 'no', 'cycleway': 'track'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'bicycle': 'use_sidepath', 'bicycle:backward': 'yes', 'cycleway:left': 'lane'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'bicycle': 'use_sidepath', 'cycleway:left': 'lane'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'bicycle': 'use_sidepath', 'cycleway:left': 'none'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'bicycle': 'use_sidepath', 'cycleway': 'lane'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'bicycle': 'use_sidepath', 'cycleway': 'no'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'cycleway': 'separate', 'highway': 'cycleway'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'bicycle': 'use_sidepath', 'highway': 'residential'}, [0]), expected={'class': 9001002, 'subclass': 340613033}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'living_street', 'maxspeed': '15 mph'}, [0]), expected={'class': 9001002, 'subclass': 1040857321}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'living_street', 'maxspeed': '20 mph'}, [0]), expected={'class': 9001002, 'subclass': 1040857321}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'living_street', 'maxspeed': '20'}, [0]), expected={'class': 9001002, 'subclass': 1040857321}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'highway': 'living_street', 'maxspeed': '30'}, [0]), expected={'class': 9001002, 'subclass': 1040857321}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'living_street', 'maxspeed': 'walk'}, [0]), expected={'class': 9001002, 'subclass': 1040857321}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'highway': 'living_street'}, [0]), expected={'class': 9001002, 'subclass': 1040857321}, disallowed_str_in_text = ['{', '}'])
        self.check_not_err(n.way(data, {'piste:grooming': 'classic', 'piste:type': 'nordic'}, [0]), expected={'class': 9001001, 'subclass': 716854348}, disallowed_str_in_text = ['{', '}'])
        self.check_err(n.way(data, {'piste:type': 'nordic'}, [0]), expected={'class': 9001001, 'subclass': 716854348}, disallowed_str_in_text = ['{', '}'])
