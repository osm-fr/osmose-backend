...
class Name_ShouldBeHousenumber(Plugin):
    ...
    def init(self, logger):
        ...
        self.errors[804] = self.def_class(item = 5010, level = 1, tags = ['name', 'fix:chair'],
            ...
            detail = T_(
'''This residential building is tagged with a name which contains only numbers.
...
'''While uncommon, it is possible for a name to be only numbers.
 This is particularly the case for some brands or amenities.''')
        ...
        self.residential_buildings = ["house", "residential", "apartments", "detached", "terrace", "dormitory", "semi", "semidetached_house"]

    def node(self, data, tags):
        err = []
        if u"building" in tags:
            if tags["building"] in self.residential_buildings:
                if u"name" in tags:
...
class Test(TestPluginCommon):
    def test(self):
        ...
        a.init(None)
        for t in [{u"building": u"house", u"name": "123"},
        ...
        for t in [{u"building": u"house", u"name": "12 Flats"},
                  {u"building": u"retail", u"name": "21"},
                  {u"building": u"semi", u"name": "25a"},
                 ]:
