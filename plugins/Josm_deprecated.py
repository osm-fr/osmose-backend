...
class Josm_deprecated(Plugin):
    ...
    def node(self, data, tags):
        ...
        if (u'access' in keys) or (u'aerialway' in keys) or (u'amenity' in keys) or (u'barrier' in keys) or (u'craft' in keys) or (u'leisure' in keys) or (u'place' in keys) or (u'service' in keys) or (u'shop' in keys) or (u'traffic_calming' in keys):
            ...
            if not match:
                capture_tags = {}
                try: match = (mapcss._tag_capture(capture_tags, 0, tags, u'shop') == mapcss._value_capture(capture_tags, 0, u'yes') and mapcss._tag_capture(capture_tags, 1, tags, u'amenity') != mapcss._value_capture(capture_tags, 1, u'fuel'))
                except mapcss.RuleAbort: pass
            if not match:
                capture_tags = {}
