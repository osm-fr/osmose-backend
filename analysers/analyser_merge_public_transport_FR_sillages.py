...
class Analyser_Merge_Public_Transport_FR_sillages(Analyser_Merge_Point):
    def __init__(self, config, logger = None):
        ...
        place = "Sillages"
        self.def_class_missing_official(item = 8040, id = 121, level = 3, tags = ['merge', 'public transport', 'fix:survey', 'fix:picture'],
            title = T_('{0} stop not integrated', place))
        self.def_class_possible_merge(item = 8041, id = 123, level = 3, tags = ['merge', 'public transport', 'fix:chair'],
            title = T_('{0} stop, integration suggestion', place))
        self.def_class_update_official(item = 8042, id = 124, level = 3, tags = ['merge', 'public transport', 'fix:chair'],
