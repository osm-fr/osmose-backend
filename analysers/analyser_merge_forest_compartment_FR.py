...
class Analyser_Merge_Forest_Compartment_FR(Analyser_Merge_Point):
    ...
    def __init__(self, config, logger = None):
        ...
        self.def_class_missing_official(item = 8550, id = 1, level = 3, tags = ['merge', 'boundary', 'fix:survey', 'fix:imagery'],
            ...
            trap = T_('Do not import the POI created by Osmose to OSM. It is just there to help you locate the missing forest compartment.'))
        ...
        self.init(
            ...
            Load_XY(('ST_X(ST_PointOnSurface(geom))',), ('ST_Y(ST_PointOnSurface(geom))',)),
            Conflate(
                select = Select(
                    types = ['nodes', 'polygons'],
                    tags = {'boundary': 'forest_compartment'}),
                conflationDistance = 0,
                mapping = Mapping(
