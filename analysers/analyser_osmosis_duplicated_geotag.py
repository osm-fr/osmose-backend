...
cvqnotag AS (
    SELECT
        ...
        CASE
            WHEN ST_X(ST_StartPoint(linestring)) = ST_X(ST_StartPoint(ST_Reverse(linestring))) THEN
                CASE
                    WHEN ST_Y(ST_StartPoint(linestring)) < ST_Y(ST_StartPoint(ST_Reverse(linestring))) THEN linestring
                    ELSE ST_Reverse(linestring)
                END
            WHEN ST_X(ST_StartPoint(linestring)) < ST_X(ST_StartPoint(ST_Reverse(linestring))) THEN linestring
            ELSE ST_Reverse(linestring)
        END as linestring,
        sha224(ST_AsBinary(
            CASE
