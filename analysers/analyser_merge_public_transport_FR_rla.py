...
###########################################################################
##                                                                       ##
## Copyrights Théo Peltier Rodrigo 2022
##                                                                       ##
...
class Analyser_Merge_Public_Transport_FR_rla(Analyser_Merge_Point):
    def __init__(self, config, logger = None):
        Analyser_Merge_Point.__init__(self, config, logger)
        place = "Lignes d'Azur"
        self.def_class_missing_official(item = 8040, id = 111, level = 3, tags = ['merge', 'public transport', 'fix:survey', 'fix:picture'],
            title = T_('{0} stop not integrated', place))
        self.def_class_possible_merge(item = 8041, id = 113, level = 3, tags = ['merge', 'public transport', 'fix:chair'],
            title = T_('{0} stop, integration suggestion', place))
        self.def_class_update_official(item = 8042, id = 114, level = 3, tags = ['merge', 'public transport', 'fix:chair'],
        ...
        self.init(
            ...
            GTFS(Source(attribution = "Régie Ligne d'Azur", millesime = "10/2022",
                fileUrl = "http://opendata.nicecotedazur.org/data/storage/f/gtfs1666394101/GTFSExport.zip")),
