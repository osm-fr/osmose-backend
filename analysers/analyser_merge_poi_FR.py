...
class Analyser_Merge_Reservoir_FR(Analyser_Merge_Dynamic):
    ...
    def __init__(self, config, logger = None):
        ...
        dict(
            ...
            id = 10700,
            level = 3,
            title = T_('Megalith not integrated'),
        ...
        for r in maping:
            self.classFactory(SubAnalyser_Merge_Reservoir_FR, r['id'], r['item'], r['id'], r['level'], r['title'], r['select'], r['tags'], r.get('generate_tags', r['tags']), r.get('height', True))
    ...
    def __init__(self, config, error_file, logger, item, id, level, title, select, tags, generate_tags, height = True):
        ...
        self.init(
            ...
            GPKG(Source(attribution = "IGN", millesime = "01/2021",
                    file = "zone_d_activite_ou_d_interet.gpkg")),
            LoadGeomCentroid(
                select = select),
            Mapping(
                select = Select(
